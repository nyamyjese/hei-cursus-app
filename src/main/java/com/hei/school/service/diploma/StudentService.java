...
public class StudentService {
    ...
    public boolean isGraduated(UUID studentId, UUID trackId) {
        ...
        for (Course course : trackCourses) {
            List<Grade> courseGrades = gradeRepository.findByStudentIdAndCourseId(studentId, course.getId());
    ...
    public boolean isYearValidated(UUID studentId, UUID trackId, int academicYear) {
        List<Course> yearlyCourses = courseRepository.findByTrackId(trackId).stream()
        ...
        BigDecimal yearlyAverage = totalScore.divide(new BigDecimal(yearlyCourses.size()), 2, java.math.RoundingMode.HALF_UP);
